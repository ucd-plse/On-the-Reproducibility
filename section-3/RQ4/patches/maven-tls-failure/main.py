...
_gradle_plugin_run_through = None
...
def _print_usage():
    print('Usage: python3 main.py <image_tags_file> <gradle_plugin_run_through>')
    print('image_tags_file: Path to a file containing a newline-separated list of image tags to process.')
    print('gradle_plugin_run_through: Set to either "True" or "False" for patching the Gradle Plugin.')
...
def _validate_input(argv):
    ...
    gradle_plugin = argv[2].lower() == 'true'
    if not os.path.isfile(image_tags_file):
        ...
        sys.exit(1)
    if gradle_plugin:
        global _gradle_plugin_run_through
        _gradle_plugin_run_through = gradle_plugin
...
def _copy_files_to_container(image_tag, f_or_p):
    ...
    if f_or_p is not None:
        ...
        if _gradle_plugin_run_through:
            # Replace the project's path to the project you're currently running
            # Example: {}:{}/build/failed/raphw/byte-buddy/byte-buddy-gradle-plugin/
            _, stdout, stderr, ok = _run_command(
                'docker cp pom.xml {}:{}/build/{}/{}/{}/{}}/'
                .format(container_id, _TRAVIS_DIR, f_or_p))
            if ok:
                print('Copied replacement pom.xml into container.')
            else:
                _print_error(
                    'Error copying replacement pom.xml into container.')
    else:
        _, container_id, _, _ = _run_command(
        ...
        if _gradle_plugin_run_through:
            # Replace the project's pom.xml path to the project you're currently running
            # Example: {}:{}/build/failed/raphw/byte-buddy/byte-buddy-gradle-plugin/
            _, stdout, stderr, ok = _run_command(
                'docker cp pom.xml {}:{}/build/failed/{}/{}/{}}/'
                .format(container_id, _TRAVIS_DIR))
            if ok:
                print('Copied replacement pom.xml into failed directory of container.')
            else:
                _print_error(
                    'Error copying replacement pom.xml into container.')

            _, stdout, stderr, ok = _run_command(
                'docker cp pom.xml {}:{}/build/passed/{}/{}/{}}/'
                .format(container_id, _TRAVIS_DIR))
            if ok:
                print('Copied replacement pom.xml into passed directory of container.')
            else:
                _print_error(
                    'Error copying replacement pom.xml into container.')
    return container_id
...
def _run_patch(container_id, repo):
    ...
    if ok:
        ...
        project_root = '{}/build/failed/{}'.format(_TRAVIS_DIR, repo)
        travis_file_path = '{}/travis.sh'.format(project_root)
        _, stdout, stderr, ok = _run_command(
            'docker exec {} python {}/patch_build_script.py {} failed {} {} False'
            .format(container_id, _TRAVIS_DIR, _FAILED_BUILD_SCRIPT, travis_file_path, project_root))
        if ok:
            print(stdout)
            ...
            _print_error('Error running {} on container {} for failed'.format(_PROCESS_SCRIPT, container_id), stdout,
                         stderr)
        project_root = '{}/build/passed/{}'.format(_TRAVIS_DIR, repo)
        travis_file_path = '{}/travis.sh'.format(project_root)
        _, stdout, stderr, ok = _run_command(
            'docker exec {} python {}/patch_build_script.py {} passed {} {} False'
            .format(container_id, _TRAVIS_DIR, _PASSED_BUILD_SCRIPT, travis_file_path, project_root))
        if ok:
            print(stdout)
...
def _run_patch_and_build(container_id, f_or_p, repo):
    ...
    :return: None
    """
    project_root = '{}/build/{}/{}'.format(_TRAVIS_DIR, f_or_p, repo)
    travis_file_path = '{}/travis.sh'.format(project_root)
    ...
    if ok:
        if f_or_p == 'failed':
            print('Successfully changed permissions container {}'.format(container_id))
            _, stdout, stderr, ok = _run_command(
                'docker exec {} python {}/patch_build_script.py {} failed {} {} True'
                .format(container_id, _TRAVIS_DIR, _FAILED_BUILD_SCRIPT, travis_file_path, project_root))
            if ok:
                print(stdout)
                print('Ran {} on container {} for failed'.format(
            ...
            _, stdout, stderr, ok = _run_command(
                'docker exec {} python {}/patch_build_script.py {} passed {} {} True'
                .format(container_id, _TRAVIS_DIR, _PASSED_BUILD_SCRIPT, travis_file_path, project_root))
            if ok:
                print(stdout)
                print('Ran {} on container {} for passed'.format(
...
def _remove_files(container_id, f_or_p):
    ...
    """
    if f_or_p == 'failed':
        _, stdout, stderr, ok = _run_command('docker exec {} sudo rm {}/patch_build_script.py {}/log-failed.log'
                                             .format(container_id, _TRAVIS_DIR, _TRAVIS_DIR))
        if ok:
        ...
        _, stdout, stderr, ok = _run_command('docker exec {} sudo rm {}/patch_build_script.py {}/log-passed.log'
                                             .format(container_id, _TRAVIS_DIR, _TRAVIS_DIR))
        if ok:
            print('Successfully removed files from {}-{}'.format(container_id, f_or_p))
        else:
        ...
        _, stdout, stderr, ok = _run_command('docker exec {} sudo rm {}/patch_build_script.py'
                                             .format(container_id, _TRAVIS_DIR))
        if ok:
            print('Successfully removed files from {}-{}'.format(container_id, f_or_p))
        else:
    ...
    for image_tag in image_tags:
        try:
            ...
            if failed_job_reproduced_result[0] and passed_job_reproduced_result[0]:
                ...
                _BUGSWARMAPI.set_artifact_failed_patch(image_tag, 'mvn-tls')
                _BUGSWARMAPI.set_artifact_passed_patch(image_tag, 'mvn-tls')

                if _gradle_plugin_run_through:
                    _BUGSWARMAPI.set_artifact_failed_patch(
                        image_tag, 'gradle-plugin')
