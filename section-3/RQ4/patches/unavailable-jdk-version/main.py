...
_JAVA_PACKAGE_PATH = './JDKs/'
_JVM_PATH = '/usr/lib/jvm/'
fail_to_fix = {}
...
def _copy_files_to_container(image_tag, f_or_p, java_version):
    """
    ...
    if f_or_p is not None:
        ...
        # copy java jdk
        _, stdout, stderr, ok = _run_command(
            'docker cp {}/{} {}:{}'.format(_JAVA_PACKAGE_PATH, java_version, container_id, _JVM_PATH))
        if ok:
            print('Copied {} into container'.format(java_version))
        else:
            _print_error('Error copying {} into container for {}'.format(
                java_version, image_tag), stdout, stderr)
            fail_to_fix[image_tag] = stderr
...
def _run_patch(container_id):
    ...
    if ok:
        ...
        _, stdout, stderr, ok = _run_command(
            'docker exec {} python {}/patch_build_script.py {} failed False'.format(container_id, _TRAVIS_DIR,
                                                                                    _FAILED_BUILD_SCRIPT))
        if ok:
            ...
            _print_error('Error running {} on container {} for failed'.format(_PROCESS_SCRIPT, container_id), stdout,
                         stderr)
        _, stdout, stderr, ok = _run_command(
            'docker exec {} python {}/patch_build_script.py {} passed False'.format(container_id, _TRAVIS_DIR,
                                                                                    _PASSED_BUILD_SCRIPT))
        if ok:
...
def _run_patch_and_build(container_id, f_or_p):
    ...
    if ok:
        if f_or_p == 'failed':
            print('Successfully changed permissions container {}'.format(container_id))
            _, stdout, stderr, ok = _run_command(
                'docker exec {} python {}/patch_build_script.py {} failed True'.format(container_id, _TRAVIS_DIR, _FAILED_BUILD_SCRIPT))
            if ok:
                print('Ran {} on container {} for failed'.format(
                    _PROCESS_SCRIPT, container_id))
            else:
            ...
            _, stdout, stderr, ok = _run_command(
                'docker exec {} python {}/patch_build_script.py {} passed True'.format(container_id, _TRAVIS_DIR, _PASSED_BUILD_SCRIPT))
            if ok:
                print('Ran {} on container {} for passed'.format(
                    _PROCESS_SCRIPT, container_id))
            else:
...
def _remove_files(container_id, f_or_p):
    ...
    if f_or_p == 'failed':
        _, stdout, stderr, ok = _run_command(
            'docker exec {} sudo rm {}/patch_build_script.py {}/log-failed.log'.format(container_id, _TRAVIS_DIR, _TRAVIS_DIR))
        if ok:
        ...
        _, stdout, stderr, ok = _run_command(
            'docker exec {} sudo rm {}/patch_build_script.py {}/log-passed.log'.format(container_id, _TRAVIS_DIR, _TRAVIS_DIR))
        if ok:
            print('Successfully removed files from {}-{}'.format(container_id, f_or_p))
        else:
            _print_error(
        ...
        _, stdout, stderr, ok = _run_command(
            'docker exec {} sudo rm {}/patch_build_script.py'.format(container_id, _TRAVIS_DIR))
        if ok:
            print('Successfully removed files from {}-{}'.format(container_id, f_or_p))
        else:
            _print_error(
...
def _patch_artifact_and_run(image_tag):
    ...
    with open(passed_job_orig_log_path) as file:
        for line in file:
            # java version "1.8.0_101"
            match = re.search(r'java version \".*\"', line)
            if match:
                java_version = re.findall('"([^"]*)"', match.group(0))[0]

    print('The correct version is {}.'.format(java_version))
    ...
    for fail_or_pass in ['failed', 'passed']:
        ...
        container_id = _copy_files_to_container(
            image_tag, fail_or_pass, java_version)
        # Replace original jdk in container
        _replace_original_jdk(container_id, java_version)
...
def _replace_original_jdk(container_id, java_version):
    origin_package_name = ""
    if "1.7" in java_version:
        origin_package_name = "java-7-oracle"
    if "1.8" in java_version:
        origin_package_name = "java-8-oracle"
    _, stdout, stderr, ok = _run_command(
        'docker exec {} sudo rm -rf /usr/lib/jvm/{}'.format(container_id, origin_package_name))
    if ok:
        print('Successfully removed files from {} in {}'.format(
            origin_package_name, container_id))
    else:
        _print_error('Error removing files from {} to {}'.format(
            container_id, origin_package_name), stdout, stderr)

    _, stdout, stderr, ok = _run_command(
        'docker exec {} sudo mv /usr/lib/jvm/{} /usr/lib/jvm/{}'.format(container_id, java_version,
                                                                        origin_package_name))
    if ok:
        print('Successfully rename files in {} from {} to {}'.format(
            container_id, java_version, origin_package_name))
    else:
        _print_error('Error rename files in {} from {} to {}'.format(
            container_id, java_version, origin_package_name))


def _patch_artifact(image_tag, java_version):
    """
    Creates a container, copies patch_build_script.py, runs the script to modify the run_failed/passed.sh scripts to add
    flags to the mvn commands to use TLSv1.2.
    :param image_tag: The image tag associated with the artifact on DockerHub
    ...
    # Copy files into container
    container_id = _copy_files_to_container(image_tag, None, java_version)
    # replace the original jdk in container
    _replace_original_jdk(container_id, java_version)
    ...
    for image_tag in image_tags:
        try:
            ...
            if failed_job_reproduced_result[0] and passed_job_reproduced_result[0]:
                ...
                container_id = _patch_artifact(image_tag, java_version)

                ShellWrapper.run_commands('docker commit {} bugswarm/images:{}'
                                          .format(container_id, image_tag),
                                          shell=True)
                ShellWrapper.run_commands('sudo docker push bugswarm/images:{}'
                                          .format(image_tag),
                                          shell=True)
                ...
                bugswarmapi.set_artifact_failed_patch(image_tag, 'jdk-ver')
                bugswarmapi.set_artifact_passed_patch(image_tag, 'jdk-ver')
            else:
